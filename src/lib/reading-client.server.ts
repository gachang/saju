...
export const READING_REASONING_EFFORT = "none" as const;
...
export function createReadingClient(timeout: number) {
  ...
  return { structured: { async parse<T>(request: {
    ...
    reasoning?: { effort: "none" | "low" }; store?: false; tools?: never[];
    ...
    const response = await client.chat.completions.create({
      ...
      ...(request.model.startsWith("gpt-5")
        ? { reasoning_effort: request.reasoning?.effort ?? READING_REASONING_EFFORT }
        : {}),
      store: false,
