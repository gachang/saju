...
- OpenAI 직접 Chat Completions API: 초안·보정·검수 모두 `gpt-5.6-luna`를 사용합니다. Structured Outputs로 JSON을 요청하고 서버에서 스키마·분량·근거를 검증합니다. 고정 프롬프트는 `src/lib/reading-prompt.ts`이며, 제목 후보 선택과 문단 단위 보정으로 통과한 본문을 보존합니다. [연동 상세](docs/openai-report.md).
...
- 선택한 명식의 형식은 검증하지만 원본 생일의 진위까지 서버가 인증하지는 않습니다. 기본 생성 과정에서는 출생정보·명식·보고서를 서버에 저장하지 않습니다. 완성된 보고서 화면에서는 즉시 공유할 수 있도록 이름·그룹명·선택된 명식·완성 보고서를 무작위 링크에 연결해 최대 3일 저장하며, 생년월일·출생시각·성별·달력 정보는 저장하지 않습니다.
- API 실패/분량·근거·형식 검사 실패 시 로딩 화면에서 안내하고 재시도할 수 있습니다. 완성된 AI 보고서 없이 결과 화면으로 넘어가지 않습니다.
...
과거 합성 입력 결과는 회귀 검사 fixture로만 보관하며 사용자용 예시 화면에는 노출하지 않습니다. 첫 생성은 수 분이 걸릴 수 있고 서명된 중간 결과에서 이어 처리합니다. 이전 평가 기록은 [평가 기록](docs/evals/2026-09-12.md)에 별도로 남겨 두었습니다.
