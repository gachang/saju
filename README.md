...
- Codyssey OpenAI 호환 Chat Completions API: 초안·보정·검수 모두 `gpt-5.4`를 사용합니다. 공급자가 지원하지 않는 `response_format`·`reasoning_effort`는 보내지 않습니다. JSON 형식은 프롬프트로 요청하고 서버에서 스키마·분량·근거를 검증합니다. 고정 프롬프트는 `src/lib/reading-prompt.ts`이며, 제목 후보 선택과 문단 단위 보정으로 통과한 본문을 보존합니다. [연동 상세](docs/codyssey-report.md).
- 브라우저에서 명식을 계산하며 API에는 선택한 명식과 호칭의 글자 수만 전달합니다. 서버가 점수와 승인된 근거를 다시 구성한 뒤 Codyssey에는 근거·점수·범위·호칭 길이만 보냅니다. 원본 생일/호칭/성별/그룹명은 전송하지 않습니다.
...
- 요청당 220초 제한, 장 보정 동시 실행은 2개입니다. 서명된 중간 결과로 최대 3단계 자동 이어 처리를 하며, 검수 미통과 시 실패를 안내합니다. 인증·잔액 오류는 반복하지 않습니다. 일시적인 429만 최대 2회 재시도합니다.
...
pnpm lint
pnpm build
# CODYSSEY_API_KEY를 실행 환경에 제공한 상태에서만, 합성 입력 4회 테스트
...
공개 AI API는 기본 비활성입니다. Vercel에 서버 전용 비밀변수 `CODYSSEY_API_KEY`와 `READING_API_ENABLED=true`를 설정하고 재배포해야 합니다. 키는 Git이나 문서에 포함하지 않습니다. 기존 `OPENAI_API_KEY`는 프로덕션 보고서에서 사용하지 않습니다. 인스턴스별 호출 제한은 글로벌 비용 한도가 아니므로 상용 공개 전 분산 호출 제한·인증·비용 한도를 추가해야 합니다. 실제 호칭 대신 NFC 기준 길이(각 1~12자)를 전송해 치환 후 분량을 검사합니다.

API 키를 Git에 커밋하지 마세요. `.env.local`은 무시되며 `.env.example`은 변수명만 포함합니다. 데이터는 Codyssey를 거쳐 처리되므로 해당 공급자의 보관·처리 정책을 별도로 확인해야 합니다.
