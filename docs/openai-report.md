...
초안·보정·검수는 모두 Structured Outputs를 지원하는 `gpt-5.6-luna`를 `reasoning_effort: "none"`으로 사용합니다. 더 낮은 비용의 `gpt-4o-mini`도 운영 테스트했지만, 이 보고서의 길이·근거·문체 품질 검사를 제한된 보정 횟수 안에 통과하지 못해 적용하지 않았습니다.
